            ...
            if st.button('Login 🚀') and hf_email and hf_pass and hf_token: 
                with st.spinner('🚀 Logging in...'):
                    ...
                    if 'generated' not in st.session_state:
                        st.session_state['generated'] = ["I'm **Concord AI chat**, How may I help you ? "]
...
# POSTGRESS PLUGIN
        if st.session_state['plugin'] == "📋 Talk with your DB" and 'pgdf' not in st.session_state:
            with st.expander("📋 Talk with your DB", expanded= True):
                conn = pg.connect("host=localhost dbname=testdb user=neeravsalaria")
                #conn = st.connection("postgresql", type="sql")
                dataframe = psql.read_sql('SELECT * FROM inventorysmall', conn)
                #pgdf = conn.query('SELECT * FROM inventory;', ttl="10m")
                st.session_state['pgdf'] = dataframe
                st.experimental_rerun()
        if st.session_state['plugin'] == "📋 Talk with your DB":
            if st.button('🛑📋 Remove DB from context'):
                if 'pgdf' in st.session_state:
...
# END OF PLUGIN
    ...
    html_chat = '<center><h6>🤗 Demo LLMAI Project🤗</h6>'
    html_chat += '<br><img src="https://concordusa.com/themes/oneconcord/images/Concord-Horizontal-Logo.svg" width="250" height="50"/><center><br>'
    st.markdown(html_chat, unsafe_allow_html=True)
    st.write('Made with ❤️ by SwiftWin')
...
with data_view_container:
    ...
    if 'pgdf' in st.session_state:
        with st.expander("🤖 View your **DB**"):
            st.data_editor(st.session_state['pgdf'], use_container_width=True)
    if 'pdf' in st.session_state:
...
def generate_response(prompt):
    ...
    source = ""
    #data = {
       # "style": ["tshirt", "sweater", "jacket", "shirt", "tshirt", "shirt"],
      #  "material": ["cotton", "polyster", "wool", "cotton", "cotton", "cotton"],
    ...
    with loading_container:
        ...
        elif st.session_state['plugin'] == "📋 Talk with your DB" and 'pgdf' in st.session_state:
            #get only last message
            context = f"User: {st.session_state['past'][-1]}\nBot: {st.session_state['generated'][-1]}\n"
            if prompt.find('python') != -1 or prompt.find('Code') != -1 or prompt.find('code') != -1 or prompt.find('Python') != -1:
                with st.spinner('🚀 Using tool for python code...'):
                    solution = "\n```python\n" 
                    solution += st.session_state['pgdf'].sketch.howto(prompt, call_display=False)
                    solution += "\n```\n\n"
                    final_prompt = prompt4Code(prompt, context, solution)
            else:  
                with st.spinner('🚀 Using tool to get information...'):
